...
{   
    if(this.is_init)
    {
        ...
        {
            var period = 4.0;
            if (this.frame % period == (Math.floor(Math.random()*period)))
            {
                _bass   = Math.random()*0.6;
                _mid    = Math.random()*0.6;
                _high   = Math.random()*0.6;
            }
        }

        this.bass   = this.bass > _bass ? this.bass * .96 : _bass;  //fade out over subsequent updates
        this.mid    = this.mid  > _mid  ? this.mid  * .90 : _mid;
        this.high   = this.high > _high ? this.high * .96 : _high;

        this.level = (this.bass + this.mid + this.high)/3.;

        this.history += this.level * .001 + .005; 
    }
