...
ThreeSharedRenderer.prototype.ziggle_cam = function(_frame, x, y)
{
    var mouse = new THREE.Vector3(-x, -y, 0);
    this.campos.lerp(mouse, 0.1);
    var _e = _frame;
    var _n_loc = new THREE.Vector3(
        Math.sin(_e)/4,
        Math.cos(_e*.7)*Math.sin(_e*.9),
        Math.cos(_e)+3).add(this.campos).normalize();
...
{
    // _target.insertBefore(this.renderer.domElement, _target.firstChild);
