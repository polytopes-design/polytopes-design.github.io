...
float snoise(vec3 v)
{
    ...
    return (42.0 * dot(m, px) + 1.) * .2085;       //todo aaron, changing this 0.5 to 0.15 looks sick.
}
...
void main()
{
	...
	vec3 m_noise_seed = position.xyz + u_audio_history;
	float m_noise_complexity = .1;
	float m_noise_time = u_audio_history * .9;
	float m_noise_scale = 11.2 + m_level;
    
	vec3 m_tangent_vector      = .00001 * norm(cross(position, vec3(1., 0., 0.))	+ cross(position, vec3(0., 1., 0.)));
    ...
    const int m_noise_oct = 4;
    for(int i = 0; i < m_noise_oct; i++)
    {
    	m_fbm              += snoise((m_noise_seed)                        * m_noise_complexity * float(i) + m_noise_time * float(i));
    	m_fbm_tangent      += snoise((m_noise_seed + m_tangent_vector)     * m_noise_complexity * float(i) + m_noise_time * float(i));
    	m_fbm_bitangent    += snoise((m_noise_seed + m_bitangent_vector)   * m_noise_complexity * float(i) + m_noise_time * float(i));
    }
    ...
    vec3 m_pos              = position 
    //vec3(morphTarget0.x, morphTarget0.y, morphTarget0.z);
    // morphTarget0;
    // position;// + norm(position)
     // + position * m_fbm * m_noise_scale *0.01
     // + (morphTarget0 - position) * morphTargetInfluences[ 0 ]
     ;
    ...
    v_noise = m_noise_col + m_noise_col * m_level * 3.62;     
#if defined(IS_WHITE)
    v_noise = 0.1 * length(position.xy);
#endif

    // rand direction
    float _dirx = snoise(m_pos.zyx * 0.00014 + m_noise_time * .00001);
	float _diry = snoise(m_pos.yzx * 0.00014 + m_noise_time * .00001);
	float _dirz = snoise(m_pos.zxy * 0.00014 + m_noise_time * .00001);
	vec3 _rand_point_dir = vec3(_dirx, _diry, _dirz);
	_rand_point_dir = 0.51 * snoise(m_pos.zxy + m_noise_time * 1.) * 2.*_rand_point_dir;
